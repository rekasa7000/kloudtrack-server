...
// * REQUEST ACTIVATION
export const requestActivation = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const activateStation = await StationService.requestActivation(
      stationId,
      activationKey
    );
    if (!activateStation) {
...
export const requestDeactivation = asyncHandler(
  async (req: Request, res: Response) => {
    const { Key, id } = req.body;
    ...
    const deactivateStation = await StationService.requestDeactivation(
      stationId,
      deactivationKey
    );
