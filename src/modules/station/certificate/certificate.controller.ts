...
export const getRootCertificate = asyncHandler(async (req: Request, res: Response) => {
...
export const createRootCertificate = asyncHandler(async (req: Request, res: Response) => {
...
export const updateRootCertificate = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const deleteRootCertificate = asyncHandler(async (req: Request, res: Response) => {
...
export const activateRootCertificate = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const listRootCertificates = asyncHandler(async (req: Request, res: Response) => {
  ...
  return sendResponse(res, formattedCertificates, 200, "Root certificates fetched successfully");
});
...
export const uploadCertificate = asyncHandler(async (req: Request, res: Response) => {
  if (req.body.certificateContent || req.body.privateKeyContent) {
    ...
    if (!certificateContent || !privateKeyContent) {
      throw new AppError("Both Certificate content and private key content are required", 400);
    }
  ...
  const fingerprint = crypto.createHash("sha256").update(certificateFile).digest("hex");
...
export const updateCertificate = asyncHandler(async (req: Request, res: Response) => {
  ...
  const files = req.files as { [fieldname: string]: Express.Multer.File[] };
  if (!files[certFieldName] && !files[keyFieldName] && !certificateId && !certificateArn && !status) {
...
export const deleteCertificate = asyncHandler(async (req: Request, res: Response) => {
  ...
  sendResponse(res, null, 200, `Certificates for thing ${station.stationName} deleted successfully`);
});

// * GET CERTIFICATE BY Station Id
export const getCertificateByStationId = asyncHandler(async (req: Request, res: Response) => {
  const stationId = +req.params.stationId;
